...
Session.getLength = function (fn) {
    // TODO this method should be use to know range of session list
};
...
Session.prototype.remove = function (fn) {
    db.lrem('sessions',-2, JSON.stringify(this), function (err, count) {
        if(err) return fn(err);
...
Session.prototype.update = function (fn) {
    ...
    db.set('session:name:' + session.name , session.id, function (err) {
        if(err) return fn(err);
        db.hmset('session:id:' + id, session, function (err) {
            if(err) return fn(err);
